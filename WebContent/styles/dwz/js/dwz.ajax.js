...
function navTabSearch(form, navTabId){
	...
	$.table.setCurrent($form.attr('data-target'));
	$.table.refreshCurrent($form.attr('action'), query);
	return false;
}
/**
...
function ajaxTodo(url, callback,flag){
	var $callback = callback || dialogReloadCallback;
	if (! $.isFunction($callback)) $callback = eval('(' + callback + ')');
	$.ajax({
		type:'POST',
		url:url,
		dataType:"json",
		async:undefined == flag || flag ? true : false,
		cache: false,
		success: $callback,
		error: DWZ.ajaxError
	});
			...
			$this.click(function(event){
				...
				if($this.attr("data-target")) {
					...
					if (selectId.length == 1) {
						selectId = selectId[0];
						url = url.replace(/{[A-Za-z_]+[A-Za-z0-9_]*}/,selectId);
					} else {
				...
				if (title) {
					...
					function(isConfirm){
						if (isConfirm) ajaxTodo(url, $this.attr("callback"));
					});
				} else {
