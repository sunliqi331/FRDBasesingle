...
//      alert("Http status: " + xhr.status + " " + xhr.statusText + "\najaxOptions: " + ajaxOptions + "\nthrownError:"+thrownError + "\n" +xhr.responseText);
  ...
  ajaxDone:function(json){
    if(json.statusCode == DWZ.statusCode.error) {
      ...
      swal({
        title: json.message || DWZ.msg("sessionTimout"),
        type: "warning",
        ...
        // 超时后，跳转到登录画面
        // if (isConfirm) DWZ.loadLogin();
      });
    } else if (json.statusCode == DWZ.statusCode.forbidden) {
        swal({
          title: json.message || DWZ.msg("forbidden"),
          type: "warning",
          showCancelButton: true,
          confirmButtonColor: '#DD6B55',
          confirmButtonText: '确认',
...
//            $this.html(response);
  ...
  $.extend(String.prototype, {
    ...
    replaceTm:function(str){
      if (!str) return this;
      return this.replace(RegExp("({[A-Za-z_]+[A-Za-z0-9_]*})","g"),str);
    },
