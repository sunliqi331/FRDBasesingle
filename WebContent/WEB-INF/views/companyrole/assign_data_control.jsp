        ...
        <c:forEach var="item" items="${mpMap }">
          ...
          <c:forEach var="rp" items="${item.value }">
            <tr id="tr_${item.key.id}" pid="${item.key.id }">
              ...
              <td>
                <a class="btnLook icon-plus-sign-alt" target="dialog" rel="addDataCtl" refresh="true" href="${contextPath }/companyRole/lookup?rp.id=${rp.id }&prefix=rolePermissions[${index }]." 
