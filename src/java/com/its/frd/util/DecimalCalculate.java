...
{   
	...
	public static double div(double v1,double v2,int scale){   
		...
		double zero = 0.0;
		BigDecimal big_zero = new BigDecimal(zero);
		int result = big_zero.compareTo(b2);
		if (result == 0){
			return zero;
		} else {
	...
	/**
	 * 提供（相对）精确的开方运算，由参数指定开方后
	 * 的小数点位数，四舍五入。
	 * @param v 被开方数
	 * @param scale 精度
	 * @return
	 */
	public static double sqrt(double v,int scale){
		double sqrtValue = Math.sqrt(v);
		BigDecimal bigDecimal = new BigDecimal(String.valueOf(sqrtValue)).setScale(scale, RoundingMode.HALF_UP);
		return bigDecimal.doubleValue();
	}
	/**
	 * 求平均值
	 * @param scale 保留位数
	 * @param nums 被求平均数列表，可变参数
	 * @return
	 */
	public static double getAverage(int scale,Double... nums){
		double total = 0D;
		for(double num : nums){
