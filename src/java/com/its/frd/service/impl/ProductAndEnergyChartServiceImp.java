...
//@Service
public class ProductAndEnergyChartServiceImp implements ProductAndEnergyAndDriverChartService {
	...
	//@Override
	public Map<BarKeyType, Object> getDriverStatsBarChart(Timestamp startDate, Timestamp endDate, Long driverId,
			DateType dateType) {
		if(startDate == null || endDate == null || driverId == null)
			return null;
		Map<BarKeyType, Object> resultMap = new HashMap<>();
		//设备时间段的产量计数集合
		List<MesDriverStats> statsList = driverStatsDao.findByMesDriverIdAndUpdatetimeBetween(driverId, startDate, endDate);
		//X轴数据
		...
		energyTotals.add(count);
		seriesItem.setData(energyTotals);
	...
	public Map<BarKeyType, Object> getEnergyDataForTotal(Timestamp startDate, Timestamp endDate, TypeScope typeScope,
		...
		List<String> xAxis_data = new ArrayList<>();
		xAxis_data.add(this.getEnergyName(energyType));
		//根据设备,时间查询时间段内的能耗集合
		 ...
		 * 1.组装series中data集合数据;
		 * 2.每次只单独统计一种能耗
		 * 3.根据energyType循环energyList,累加
		 
		BarChart.SeriesItem seriesItem = new BarChart().new SeriesItem();
		seriesItem.setName(this.getEnergyName(energyType));
		List<Double> energyTotals = new ArrayList<Double>();
		energyTotals.add(this.getEnergyTotalForEnergyType(energyType, energyList));
		seriesItem.setData(energyTotals);
	...
	@Override
	public Map<BarKeyType, Object> getEnergyDataForTotal(Timestamp startDate, Timestamp endDate, EnergyType energyType,
			Long driverId, DateType dateType) {
	...
	public Map<BarKeyType, Object> getEnergyDataForDriver(Timestamp startDate, Timestamp endDate, TypeScope typeScope,
		...
		seriesItem.setData(this.getEnergySeriesItemDataByEnergyType(energType, driverList, energyList));
...
//		List<MesProduction> proList = this.getProductionListByMysql(modelnum, startDate, endDate);
	 ...
	 * @return
	 */
	private String getPointUnitName(Long propertyId){
		if(propertyId == null)
			return null;
		return this.getPoint(propertyId).getUnits();
	}
	 ...
	 * @return
	 */
	private String getPointKey(Long propertyId){
		if(propertyId == null)
			return null;
		MesPoints point = this.getPoint(propertyId);
		if(point != null)
	 ...
	 * @return
	 */
	private MesPoints getPoint(Long propertyId){
		if(propertyId == null)
			return null;
		MesDriverPoints dp = driverPointDao.findByMesDrivertypePropertyId(propertyId);
		if(dp != null && dp.getMesPoints() != null)
			return dp.getMesPoints();
		return null;
	}
    ...
    * 单体版本
     ...
     */
    private List<MesProduction> getProductionListByMysql(String modelnum,Timestamp startDate,Timestamp endDate){
        ...
        if(modelnum != null && !modelnum.equals("") && !modelnum.equals("null")){
            searchList.add(new SearchFilter("productMode", Operator.EQ, String.valueOf(modelnum)));
        }
        ...
        List<MesDataProductProcedure> productList = mesDataProductProcedureService.findAll(specification);
        
//        MesDataRowkeyUtil.setEntityManager(entityManager);
//        String beginTimeS = String.valueOf(startDate.getTime()).replace("000", "");
//        String endTimeS = String.valueOf(endDate.getTime()).replace("000", "");
//
//        List<String> ngCount =MesDataRowkeyUtil.getCountByPageSelect("", "", "", String.valueOf(productId),
//                "",
...
//        outMap.put("nopassCount", Long.valueOf(totolCount != null ? ngCount.size() : 0L));
	 ...
	 * @return 时间分段字符串集合
	 */
	private List<String> getDateListStr(DateType dateType,Timestamp startDate, Timestamp endDate){
	 ...
	 * @return 时间分段集合
	 */
	private List<Timestamp> getDateList(DateType dateType,Timestamp startDate, Timestamp endDate){
	...
	private List<String> getDefineTimeForMonthStr(Timestamp start,Timestamp end){
		...
		for(Timestamp time : times){
			...
			months.add(cd.get(Calendar.MONTH) + "月");
		}
		return months;
	}
	...
	public static void main(String[] args) {
		ProductAndEnergyChartServiceImp ps = new ProductAndEnergyChartServiceImp();
	...
	public Map<BarKeyType, Object> getRuntimeDataForTotal(Timestamp startDate, Timestamp endDate, TypeScope typeScope,
			Long id, DateType dateType,String ifIsDriver) {
		// TODO Auto-generated method stub
		return null;
	}
